...
var durl = Object.values(ypsi)
.filter(x => x.includes('videoplayback?') && !x.includes('range='));

var dsig = new Set(
   Object.keys(ypsi).filter(x => x.includes(',signature')).map(x => ypsi[x])
).values();

var vqua = Object.keys(ypsi)
.filter(x => x.includes(',qualityLabel'))
